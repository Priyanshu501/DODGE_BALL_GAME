...
public class DodgeBall implements Runnable{
	...
	private void makeHarder() {
		...
		case 0: {
			ballCount++; System.out.println("Added ball."); 
			break;
			}
		default: {
			player.grow();
			System.out.println("Increased player size.");
			break;
			}
		}
	}
	...
	/**
	 *  Getting mouse location relative to upper left corner of panel.
	...
	private Point getAdjustedMouseLoc() {
		...
		return new Point (
				(int) location.getX() - frame.getX() - PANEL_OFFSET.width,
	...
	private void checkCollision() {
		for (int i = 0; i < balls.length; ++i) {
			...
			for (int j = 0; j < balls.length; ++j) {
				Ball b = balls[i];
				if (a != b && a.isColliding(b)) a.bounce(b);
			}
		...
		public DodgeBallPanel() {
			...
			addMouseListener(new MouseListener() {
				...
				public void mousePressed(MouseEvent arg0) {
					if (alive) player.setImg(winkImg);
				}
				
				@Override
				public void mouseReleased(MouseEvent arg0) {
					if (alive) player.setImg(happyImg);
				}
			});
		}
		...
		public void paintComponent(Graphics g) {
			...
			if (suspended) {
				...
				if (alive) {
					...
					g.drawString("You made it", BOUNDS.width/2 - 72, BOUNDS.height/2+80);
	...
	static {
		try {
			happyImg = ImageIO.read(new File("images/happy_face.png"));
